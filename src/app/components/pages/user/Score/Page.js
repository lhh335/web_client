import {QUERY_SCORE_STREAM_C2S,QUERY_SCORE_STREAM_S2C,} from "../../../../proto_enum";
...
class TableExampleComplex extends Component {
  ...
  getPageData = (page, notice = false) => {
    ...
    var cb = (id = 0, message = null, args) => {
      ...
      if (id !== QUERY_SCORE_STREAM_S2C) {
        return;
      }
      self.state.count = message.count;
      var result = message.ssi;
    ...
    MsgEmitter.emit(QUERY_SCORE_STREAM_C2S, obj, cb, [this, notice]);
      ...
      <div>
        ...
        <ReactDataGrid
          ...
          columns={[
            {
              key: 'coin',
              name: Lang[window.Lang].User.ScorePage.Table.score.name,
              resizable: true
            },
            {
              key: 'flag',
              name: Lang[window.Lang].User.ScorePage.Table.flag.name,
              resizable: true
            },
            {
              key: 'operation',
              name: Lang[window.Lang].User.ScorePage.Table.operation.name,
              resizable: true
            },
            {
              key: 'game',
              name: Lang[window.Lang].User.ScorePage.Table.game.name,
              resizable: true
            },
            {
              key: 'room',
              name: Lang[window.Lang].User.ScorePage.Table.room.name,
              resizable: true
            },
            {
              key: 'desk',
              name: Lang[window.Lang].User.ScorePage.Table.desk.name,
              resizable: true
            },
            {
              key: 'time',
              name: Lang[window.Lang].User.ScorePage.Table.time.name,
              resizable: true
            },
            {
              key: 'vertify',
              name: Lang[window.Lang].User.ScorePage.Table.vertify.name,
              resizable: true
            }
          ]}
          rowGetter={(i) => {
            if (i === -1) { return {} }
            return {
              coin: this.state.data[i].score,
              flag: this.state.data[i].flag === 1 ? (Lang[window.Lang].User.ScorePage.add + this.state.data[i].change_score) : (Lang[window.Lang].User.ScorePage.reduce + this.state.data[i].change_score),
              operation: Lang[window.Lang].User.ScorePage.action[this.state.data[i].operation],
