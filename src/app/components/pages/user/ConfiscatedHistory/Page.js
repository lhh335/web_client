...
class PageComplex extends React.Component {
    ...
    queryExchange = (reload = false, notice = false) => {
        ...
        if (this.state.search_way === 'account') {
            account = this.state.serchInput || document.getElementById("serch_player_text_field").value;
        } else if (this.state.search_way === 'recommend') {
            account = ""
            value = document.getElementById("serch_player_text_field").value;
        }
        if (account === "") {
            if (this.state.search_way === 'account' || value === "") {
                var searchObj = {
                    approve: 22,
    ...
    render() {
        ...
        return (
            <div>
                ...
                {player === "" ?
                    <div>
                        ...
                        >
                            ...
                            <MenuItem value={"recommend"} key={"recommend"}
                                primaryText={Lang[window.Lang].User.AccountPage.search_by_recommend} />
                        </SelectField>
                        <TextField
                            id="serch_player_text_field"
                            disabled={false}
                            multiLine={false}
                            hintText={this.state.search_way === 'account' ? Lang[window.Lang].User.AccountPage.search_by_account : Lang[window.Lang].User.AccountPage.search_by_recommend}
                            defaultValue=""
                            style={Styles.selecteField}
                            middleWidth={true}
                ...
                <CommonTable
                    table={Lang[window.Lang].User.ConfiscatedPage.Table}
                    ...
                    rowGetter={(i) => {
                        ...
                        return {
                            ...
                            recommend: this.state.data[i].recommend
                        }
                    }}
                ...
                <div style={Styles.totalApproveAndRefuse}>
                    <h5 style={Styles.fontLeftFloat}>总扣除:{this.state.total_approved}元</h5>
                </div>
...
class UserConfiscatedHistory extends Component {
    constructor(props) {
        super(props);
    }
    render() {
        return (
            <div>
                <Title render={(previousTitle) => `${Lang[window.Lang].User.player_confiscated_history} - ${previousTitle}`} />
                <PageComplex player={this.props.player} />
            </div>
        )
    }
}
export default UserConfiscatedHistory;
