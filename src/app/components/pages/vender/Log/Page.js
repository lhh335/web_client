    ...
    return (
      <div>
        ...
        <TextField
          ...
          disabled={false}
          multiLine={false}
          hintText={Lang[window.Lang].User.LogPage.search_button}
          defaultValue=""
          fullWidth={true}
        />

        <RaisedButton label={Lang[window.Lang].User.LogPage.search_button} primary={true} style={styles.simple} onMouseUp={
          () => {
            this.state.data = [];
            this.state.allData = [];
            this.setState({ currentPage: 1, totalPage: 1 });
        ...
        <ReactDataGrid
          ...
          columns={[
            {
              key: 'account',
              name: Lang[window.Lang].User.LogPage.Table.account,
              resizable: true
            },
            {
              key: 'vendoridentfier',
              name: Lang[window.Lang].User.LogPage.Table.vendoridentfier,
              resizable: true,
            },
            {
              key: 'system_version',
              name: Lang[window.Lang].User.LogPage.Table.system_version,
              resizable: true
            },
            {
              key: 'device_model',
              name: Lang[window.Lang].User.LogPage.Table.device_model,
              resizable: true
            },
            {
              key: 'device_name',
              name: Lang[window.Lang].User.LogPage.Table.device_name,
              resizable: true
            },
            {
              key: 'device_type',
              name: Lang[window.Lang].User.LogPage.Table.device_type,
              resizable: true
            },
            {
              key: 'device_uniqueidentifier',
              name: Lang[window.Lang].User.LogPage.Table.device_uniqueidentifier,
              resizable: true
            },
            {
              key: 'operating_system',
              name: Lang[window.Lang].User.LogPage.Table.operating_system,
              resizable: true
            },
            {
              key: 'ip',
              name: Lang[window.Lang].User.LogPage.Table.ip,
              resizable: true
            },
            {
              key: 'login_time',
              name: Lang[window.Lang].User.LogPage.Table.login_time,
              resizable: true
            }
          ]}
          rowGetter={(i) => {
...
const UserLog = () => (
  <div>
    <Title render={(previousTitle) => `${Lang[window.Lang].User.log} - ${previousTitle}`} />
    <TableExampleComplex />
  </div>
);
