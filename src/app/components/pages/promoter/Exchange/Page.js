import { QUERY_PROMOTER_EXCHANGE_C2S, REFUSE_PROMOTER_EXCHANGE_C2S, APPROVE_PROMOTERS_EXCHANGE_C2S, QUERY_PROMOTER_EXCHANGE_S2C, REFUSE_PROMOTER_EXCHANGE_S2C, APPROVE_PROMOTERS_EXCHANGE_S2C, } from "../../../../proto_enum";
import { NOT_ANY_MORE_DATA, LOGIC_SUCCESS, ERROR_HAVE_NO_MORE_PAGES } from "../../../../ecode_enum";
...
class PageComplex extends React.Component {
  ...
  showAll = (reload = false, notice = false) => {
    ...
    var cb = (id = 0, message = null, args) => {
      ...
      if (id !== QUERY_PROMOTER_EXCHANGE_S2C) {
        return;
      }
      ...
      if (args[2] === true) {
        if (message.code === ERROR_HAVE_NO_MORE_PAGES && result.length === 0) {
          self.popUpNotice("notice", NOT_ANY_MORE_DATA, Lang[window.Lang].Master.no_data);
        } else {
    ...
    MsgEmitter.emit(QUERY_PROMOTER_EXCHANGE_C2S, obj, cb, [this, this.state.searchInput, notice]);
  };
  ...
  refuseSelect = () => {
    ...
    var cb = function (id, message, args) {
      ...
      if (id !== REFUSE_PROMOTER_EXCHANGE_S2C) {
        return;
      }
      var self = args[0];
    ...
    MsgEmitter.emit(REFUSE_PROMOTER_EXCHANGE_C2S, obj, cb, args);
  }
  ...
  approveSelect = () => {
    ...
    var cb = function (id, message, args) {
      ...
      if (id !== APPROVE_PROMOTERS_EXCHANGE_S2C) {
        return;
      }
      var self = args[0];
      var selectedData = args[1];
    ...
    MsgEmitter.emit(APPROVE_PROMOTERS_EXCHANGE_C2S, obj, cb, args);
  }
  ...
  PasswordDialog = () => {
    return (<Dialog
      title={
        Lang[window.Lang].Promoter.ExchangePage.your_pw}
      actions={[<FlatButton
    ...
    >
      <TextField
        ...
        multiLine={false}
        type="password"
        hintText={Lang[window.Lang].Promoter.ExchangePage.your_pw}
        defaultValue=''
        fullWidth={true}
      />
    </Dialog>)
    ...
    return (
      <div>
        ...
        <div>
          ...
          <CommonTable
            table={Lang[window.Lang].Promoter.ExchangePage.Table}
...
const UserExchange = () => (
  <div>
    <Title render={(previousTitle) => `推广员兑换 - ${previousTitle}`} />
    <PageComplex />
  </div>
);
