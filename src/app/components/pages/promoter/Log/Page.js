import { QUERY_PROMOTER_LOG_C2S, QUERY_PROMOTER_LOG_S2C, } from "../../../../proto_enum";
...
class TableExampleComplex extends Component {
  ...
  getPageData = (page, notice = false) => {
    ...
    var account = this.state.promoter.account || this.state.serchInput;
    if (account === "") {
      this.state.onloading = false;
    ...
    var cb = (id = 0, message = null, args) => {
      ...
      if (id !== QUERY_PROMOTER_LOG_S2C) {
        return;
      }
      var self = args[0];
      var result = message.wl;
      ...
      self.setState({ totalPage: Util.page.getTotalPage(message.count, this.props.promoter.account === undefined ? 10 : this.state.rowsPerPage) })

      if (result.length > 0 && args[1] === true) {
        self.popUpNotice("notice", message.code, Lang[window.Lang].Master.action_success);
      } else if (result.length === 0) {
        self.popUpNotice("notice", message.code, "该推广人还没有日志");
      }
    }

    MsgEmitter.emit(QUERY_PROMOTER_LOG_C2S, obj, cb, [this, notice]);
  ...
  handleChangeSearchWay = (event, index, value) => this.setState({ selectType: value });

  LogDialog = () => {
    return (<Paper style={{ paddingTop: 10, paddingBottom: 10 }}>
  ...
  insertDetail = () => {
    var table = Lang[window.Lang].Promoter.LogPage.Table;
    var list = [];
    for (var key in table) {
      list.push(
        <ul key={key}>
          <li>
            {table[key].name}: {this.state.singleLogInfo === {} ? "" :
              key === "proto_name" ? this.makeLog(this.state.singleLogInfo) :
  ...
  render() {
    ...
    return (
      <div>
        <Paper>
          ...
          } />
          {
            this.state.showCol === "all" ?
              ...
              >
                <MenuItem value={"all"} key={"all"} primaryText={Lang[window.Lang].User.AccountPage.all} />
                {this.insertMenuItem()}
              </SelectField>
          }
        </Paper>
        ...
        <CommonTable
          table={Lang[window.Lang].Promoter.LogPage.Table}
          /*table表示表格标题*/
          ...
          rowGetter={(i) => {
            ...
            return {
              ...
              proto_name: this.makeLog(this.state.data[i]),
              flag: this.state.data[i].flag === 0 ? Lang[window.Lang].Master.action_success : Lang[window.Lang].ErrorCode[this.state.data[i].flag],
        ...
        <br />
        {(this.state.LogDetail && promoter.account === undefined) ? this.LogDialog() : ''}
        <CommonAlert
...
/*const PromoterLog = () => (
  <div>
    <Title render={(previousTitle) => `推广员日志 - ${previousTitle}`} />
    <TableExampleComplex />
  </div>
);*/
class PromoterLog extends Component {
  constructor(props) {
    super(props);
  }
  render() {
    return (
      <div>
        <Title render={(previousTitle) => `推广员日志 - ${previousTitle}`} />
        <TableExampleComplex promoter={this.props.promoter} />
      </div>
    )
  }
}
